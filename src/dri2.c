                      ...
                      unsigned int format)
{
    ...
    tegra_pixmap = exaGetPixmapDriverPrivate(pixmap);

    if (tegra_2d_surface_get_bo(tegra_pixmap->surf, &bo) == TEGRA_2D_OK) {
        drm_tegra_bo_get_name(bo, &buffer->name);
    } else {
        xf86DrvMsg(scrn->scrnIndex, X_ERROR,
                   "tegra_2d_surface_get_bo() failed\n");
    }

    tegra_2d_get_surface_pitch(tegra_pixmap->surf, &buffer->pitch);
